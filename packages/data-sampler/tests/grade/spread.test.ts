...
describe("Spread.parse", () => {
  test("reads fractions", () => {
    expect(Spread.parse("normal:0.9,ill:0.1")).toEqual({ ill: 0.1, normal: 0.9 });
  });
  ...
  test.each(["", "normal", "normal:x", "normal:0.5,normal:0.5", "normal:", "a:b:c"])(
    "rejects %j",
    (text) => {
...
describe("Spread.validate", () => {
  test("accepts weights that sum to 1", () => {
    expect(Spread.validate({ ill: 0.1, normal: 0.9 }, scale)).toEqual({ ill: 0.1, normal: 0.9 });
  });

  test("rejects a negative weight", () => {
    expect(() => Spread.validate({ ill: -0.1, normal: 1.1 }, scale)).toThrow(SpreadError);
  });

  test("rejects a sum of 0.95", () => {
    expect(() => Spread.validate({ ill: 0.05, normal: 0.9 }, scale)).toThrow(/sum to 1/);
  });

  test("rejects a key outside the scale", () => {
    expect(() => Spread.validate({ dead: 0.1, normal: 0.9 }, scale)).toThrow(/not in the scale/);
  });

  test('rejects a key named "default"', () => {
    expect(() => Spread.validate({ default: 0.1, normal: 0.9 }, scale)).toThrow(/default/);
  });
...
describe("Spread.allocate", () => {
  ...
  test("breaks a tie for a single record in favour of the earlier grade", () => {
    expect(Spread.allocate({ a: 0.5, b: 0.5 }, 1, ["a", "b"])).toEqual({ a: 1, b: 0 });
  });
...
describe("Spread.sequence", () => {
  test("shuffles deterministically and preserves counts", () => {
    ...
    const second = Spread.sequence(allocation, ["a", "b"], new ScriptedRandom([0.1, 0.9, 0.5]));
